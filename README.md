...
<p align="center">
  <img src="docs/assets/agenteval-2.png" alt="Overview tab — radar with baseline overlay, tiles, score table, deterministic catalog">
</p>
...
<p align="center">
  <img src="docs/assets/agenteval-3.gif" alt="Per-question — heatmap, conversation, tool calls, rubric verdicts">
</p>
...
<p align="center">
  <img src="docs/assets/agenteval-5.png" alt="Iteration history — trend lines per metric across runs + per-iteration delta cards">
</p>
...
<p align="center">
  <img src="docs/assets/agenteval-4.gif" alt="AI Analysis — Gemini's narrative diagnosis with code citations">
</p>
...
<p align="center">
  <img src="docs/assets/agenteval-1.png" alt="Vertex AI evaluation docs sidebar — three boxes: dataset, metrics, run">
</p>
...
| Flag | Why this value |
|---|---|
| `-a adk` | ADK Python agent — what `agent-eval` evaluates |
| `-d none` | Local-only — fastest path to start iterating; no deploy step |
| `-d cloud_run` | Cloud Run FastAPI deployment — same local pipeline plus an HTTP endpoint |
| `-d agent_engine` | Managed Vertex AI Agent Engine — also unlocks the streamlined `create_evaluation_run` pass |
| `--cicd-runner google_cloud_build` | Cloud Build needs no external accounts — easiest to bootstrap |

> Start with `-d none` or `-d cloud_run`; `agent-eval` runs the full local pipeline against either, and you can deploy to Agent Engine later (it'll be auto-detected on the next `init`).
