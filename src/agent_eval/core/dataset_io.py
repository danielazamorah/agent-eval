...
def migrate_legacy(
    ...
    *,
    ...
    """Migrate legacy ``eval/`` layout → unified ``tests/eval/dataset.jsonl``.

    Auto-detects the legacy folder at ``<agent_dir>/app/eval`` (Agent Starter
    Pack layout) or ``<agent_dir>/eval`` (flat layout). Also relocates
    ``metric_definitions.json`` to ``tests/eval/metrics/`` when present.
    Originals are copied to ``tests/eval/.backup/<timestamp>/`` when
    ``backup=True``.
    ...
    if legacy_eval is not None:
        ...
        if metrics_src_candidate.exists():
            ...
            metrics_dst = agent_dir / "tests" / "eval" / "metrics" / "metric_definitions.json"

    out_path = Path(output_path) if output_path else agent_dir / "tests" / "eval" / "dataset.jsonl"
    rows = scen_rows + golden_rows
    ...
    if backup and sources_present:
        ...
        backup_dir = agent_dir / "tests" / "eval" / ".backup" / ts
