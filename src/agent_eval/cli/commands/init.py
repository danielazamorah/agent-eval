...
def _display_next_steps(agent_name: str, agent_dir: Path, mode: str) -> None:
    ...
    if mode in ("user-sim", "both"):
        ...
        lines.append("")
        lines.append("[bold]2.[/] Run ADK simulation:")
        lines.append(f"   [dim]$[/] uv run adk eval {agent_name} <eval_set>")
        lines.append("")
        lines.append("[bold]3.[/] Convert traces to evaluation format:")
        lines.append(f"   [dim]$[/] uv run agent-eval convert --agent-dir {agent_dir} \\")
        lines.append(f"       --output-dir {eval_path}/results")
    elif mode == "diy":
