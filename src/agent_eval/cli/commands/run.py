...
@click.option("--sim-parallelism", type=int, default=_DEFAULT_SIM_PARALLELISM, show_default=True,
...
@click.option("--metrics-files", "metrics_files", multiple=True, default=None,
...
def run(agent_dir, eval_dir, run_id, run_simulate, sim_parallelism, run_interact, base_url,
        run_evaluate, metrics_files, app_name, questions_file, num_questions,
        skip_traces, run_analyze, focus, skip_gemini, run_dashboard, debug):
    ...
    if not agent_dir:
        ...
        if not agents:
            ...
            console.print(f"  [dim]Run this from your agent project, or pass --agent-dir <path>.[/]")
            sys.exit(1)
        if len(agents) > 1:
        ...
        try:
            ...
            console.print(f"  [dim]Auto-detected agent at[/] [cyan]{rel}[/]  [dim](use --agent-dir to override)[/]")
        except ValueError:
    ...
    if not dataset_path.exists():
        ...
        console.print(f"  [dim]Run `agent-eval init` to scaffold one, or `agent-eval migrate` for legacy eval/ files.[/]")
        sys.exit(1)
        ...
        if run_interact:
            ...
            if not _check_url(base_url):
                ...
                if live:
                    if len(live) == 1:
                        from rich.prompt import Confirm
                        console.print(f"  [green]Found a live agent at[/] [cyan]{live[0]}[/]")
                        ...
                        console.print("  [green]Found multiple live agents — pick one:[/]")
    ...
    if run_evaluate:
        ...
        if not metric_paths:
            ...
            console.print(f"  [dim]Evaluation will be skipped. Run `agent-eval init` to scaffold one, or pass --metrics-files <path>.[/]")
            run_evaluate = False
    ...
    if run_simulate:
        ...
        _simulate_ok = _run_simulate_phase(
            agent_name, agent_path, project_root, raw_dir,
            debug=debug, max_parallel=sim_parallelism,
        )
        ...
        if _simulate_ok:
            ...
            if not run_interact:
                console.print()
                console.print("  [red]Simulate failed and interact is disabled — nothing left to score.[/]")
                console.print("  [dim]Re-run with `--no-simulate` to scope down, or fix the failure above and try again.[/]")
                sys.exit(1)
            ...
            if not keep_going:
                ...
                sys.exit(1)
            console.print(f"  [yellow]Continuing with interact only — multi-turn metrics will be skipped.[/]")
    ...
    if run_interact:
        ...
        if interact_output:
            ...
            if not interaction_files:
                console.print("  [red]Nothing to score — simulate also produced no output.[/]")
                sys.exit(1)
            if _pauses_disabled():
                console.print("  [dim]AGENT_EVAL_NO_PAUSES=1 — continuing with simulate output only.[/]")
            else:
                from rich.prompt import Confirm
                keep_going = Confirm.ask(
                ...
                if not keep_going:
                    console.print("  [dim]Aborted. Fix the interact failure and re-run.[/]")
                    sys.exit(1)
                console.print("  [yellow]Continuing with simulate output only — single-turn metrics will be skipped.[/]")
    ...
    if auto_started_proc is not None and auto_started_proc.poll() is None:
        ...
        try:
            ...
            try:
                auto_started_proc._agent_eval_log_fp.close()  # type: ignore[attr-defined]
            except Exception:
                pass
    ...
    if run_evaluate:
        ...
        _run_evaluate_phase(interaction_files, metric_paths, run_dir, run_id, debug=debug)
        ...
        if failed_now and run_analyze:
            ...
            if _pauses_disabled():
                console.print("  [dim]AGENT_EVAL_NO_PAUSES=1 — continuing to Analyze (CI default).[/]")
            else:
                from rich.prompt import Confirm
                keep_going = Confirm.ask(
                ...
                if not keep_going:
                    console.print(f"  [dim]Aborted before Analyze. Results so far: {run_dir}[/]")
                    sys.exit(1)
    ...
    if analysis_result:
        ...
        if analysis_result.get("html_report_path"):
            output_files.append("  - report.html  [bold cyan](open this — combined view of everything above)[/]")
    ...
    phase_summary = "  ".join(phase_summary_parts) if phase_summary_parts else "(no phases ran)"
    ...
    html_path = (analysis_result or {}).get("html_report_path") if analysis_result else None
    if html_path:
        rel_html = os.path.relpath(html_path, cwd)
        body_lines.append("")
    ...
    if html_path and "View" in phases:
        _phase_header("View",
                      "Opening your report. The HTML has tabs for Overview, "
    ...
    if not run_evaluate:
        ...
        rel_metrics = " \\\n  ".join(f"--metrics-files {os.path.relpath(p, cwd)}" for p in metric_paths) if metric_paths else "--metrics-files <path>"
        console.print()
...
def _split_scenarios_into_evalsets(
    agent_path: Path, project_root: Path, run_id: str,
) -> list[tuple[str, Path]]:
...
def _create_and_load_evalset(
    agent_name: str, project_root: Path, eval_set_name: str,
    scenarios_file: Path, session_file: Path,
) -> tuple[bool, str]:
...
def _run_one_adk_eval(
    agent_name: str, agent_path: Path, project_root: Path,
    eval_set_name: str, eval_config: Path, log_path: Path | None,
    ...
    if log_path is not None:
        ...
        with open(log_path, "w", buffering=1) as fp:
            ...
            fp.flush()
            proc = subprocess.run(
                cmd, cwd=str(project_root), env=_clean_env(project_root),
                stdout=fp, stderr=subprocess.STDOUT, text=True,
            )
...
def _cleanup_sim_aux_files(
    agent_path: Path, project_root: Path, evalsets: list[tuple[str, Path]],
) -> int:
...
def _start_adk_api_server(agent_path: Path, project_root: Path, port: int) -> "subprocess.Popen":
    ...
    proc = subprocess.Popen(
        [*_UV_RUN_ADK, "adk", "web", str(project_root),
...
def _wait_for_url_ready(url: str, timeout_s: float = 60.0, interval: float = 0.5) -> bool:
...
def _run_simulate_phase(
    agent_name: str, agent_path: Path, project_root: Path, raw_dir: Path,
    debug: bool = False, max_parallel: int = _DEFAULT_SIM_PARALLELISM,
) -> bool:
    ...
    if cleared_history or cleared_tmp:
        bits = []
        if cleared_history:
            bits.append(f"{cleared_history} eval_history file{'s' if cleared_history != 1 else ''}")
        if cleared_tmp:
            bits.append(f"{cleared_tmp} stale .agent_eval_tmp file{'s' if cleared_tmp != 1 else ''}")
    ...
    console.print(f"  [bold]3.[/] Splitting {n_scenarios} scenario{'s' if n_scenarios != 1 else ''} into eval_sets...")
    evalsets = _split_scenarios_into_evalsets(agent_path, project_root, run_id_for_evalset)
    session_file = agent_path / "session_input.json"
    for name, scen_file in evalsets:
        ok, err = _create_and_load_evalset(agent_name, project_root, name, scen_file, session_file)
        if not ok:
            console.print(f"     [red]Failed to set up {name}:[/] {err}")
            return False
    console.print(f"     [green]+[/] Created {len(evalsets)} eval_set{'s' if len(evalsets) != 1 else ''}")
    ...
    parallel_note = (
        f"in parallel (cap {actual_parallel})" if actual_parallel > 1
        else "sequentially"
    )
    console.print(f"  [bold]4.[/] Running ADK User Sim — {len(evalsets)} scenario(s) {parallel_note}...")
    ...
    if actual_parallel > 1:
        with console.status("     [dim]Warming uv resolver (one-time)...[/]", spinner="dots"):
            ...
            with ctx:
                with ThreadPoolExecutor(max_workers=actual_parallel) as pool:
                    futures = {
                        pool.submit(
                            _run_one_adk_eval,
                            agent_name, agent_path, project_root, name, eval_config,
    ...
    if failures and not has_traces:
        console.print(f"     [red]ADK eval failed across all scenarios — no traces generated.[/]")
    ...
    if failures:
        ...
        for name, rc, _ in failures[:3]:
            console.print(f"     [dim]Failed: {name} (rc={rc}) — see sim_logs/ if --debug[/]")
    console.print(f"     [dim]Wall-clock: {elapsed_str} for {len(evalsets)} scenario(s) "
    ...
    if n_cleaned:
        console.print(f"     [dim]Cleaned up {n_cleaned} transient eval_set file(s) from {agent_path.name}/[/]")
    ...
    if result.returncode != 0 and has_traces:
        console.print(f"     [yellow]![/] ADK eval exited non-zero, but trace files were captured.")
        console.print(f"     [dim]Use --debug to see ADK's full stderr (deprecation/EXPERIMENTAL warnings, "
    ...
    try:
        ...
        converter = AdkHistoryConverter(str(agent_path), None, prompt_to_reference=prompt_to_ref)
...
def _run_interact_phase(
    ...
    try:
        ...
        if qf.suffix.lower() == ".jsonl":
            ...
            with open(qf) as _f:
                _data = json.load(_f)
            _q_count = len(_data.get("questions") or _data.get("golden_questions") or [])
...
def _run_evaluate_phase(
    interaction_files: list[Path], metric_paths: list[Path],
