...
def _seed_legacy_layout(root: Path, *, nest_under_app: bool = False) -> Path:
    ...
    (base / "scenarios" / "conversation_scenarios.json").write_text(json.dumps({
        "scenarios": [
            {"starting_prompt": "Plan a trip", "conversation_plan": "Then refine"},
        ],
    }))
    (base / "scenarios" / "session_input.json").write_text(json.dumps({
        "app_name": "my_app", "user_id": "eval_user", "state": {},
    }))
    (base / "eval_data" / "golden_dataset.json").write_text(json.dumps({
        "golden_questions": [
            {
    ...
    (base / "metrics" / "metric_definitions.json").write_text(json.dumps({
...
class TestMigrateLegacy(unittest.TestCase):
    ...
    def test_copies_metric_definitions(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            assert json.loads(dst.read_text()) == {"general_quality": {"type": "managed"}}
